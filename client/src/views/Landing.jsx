  ...
  useEffect(() => {
    const fetchResult = async () => {
      ...
      try {
        ...
        const { data: { data: results } } = await axios.get(`${process.env.API_URL}=${url}`)
    ...
    <div className="flex bg-purple-1000 h-screen w-screen flex-col flex-1">
      ...
      <div className="flex flex-col flex-1 xl:mx-80">
        <div className="flex flex-col">
          ...
          {isDismissed ? <></> : <div className="bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md mr-20 ml-2" role="alert">
            <div className="flex">
              <div className="py-1"><svg className="fill-current h-6 w-6 text-red-500" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" onClick={() => setIsDismissed(!isDismissed)}>
          ...
          <div className="flex mt-5 ml-2">
            <input type="search" name="search" id="songsearch" placeholder="Paste song URL here."
              className="bg-purple-1000 focus:outline-none text-purple-1500 w-11/12 border-yellow-700 border border-l-0" onChange={e => setSrcTrack(e.target.value)} />
            <button type="button" className="bg-yellow-700 text-white font-bold py-1 px-4 border border-yellow-700 rounded mb-2 mx-2 md:mr-32 mt-2" onClick={e => {
              setLoading(true)
              setQrl(`https://38ef0b6cbefe.ngrok.io/api/v1/search?track=${qrl}`)
            }}>Search</button>
          </div>

          {tracks.length == 0 ? <span className="justify-center text-center text-purple-500 mt-10">An empty void..</span> : (loading == true ? <span className="text-center justify-center text-red-700">Loading</span> : tracks.map((x, y) => {
            return (
